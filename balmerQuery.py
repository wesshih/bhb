...
def makeQueries(bhb_fits,fields):
	...
	while index < len(bhb_fits):
		...
		q_str = q_str + ' FROM sppLines lines WHERE '
		q_str = q_str + '(lines.bestObjID = ' + `bhb_fits[index]['spec.bestObjID']` + ' AND lines.specObjID = ' + `bhb_fits[index]['spec.specObjID']` + ')' 
		for bhb in bhb_fits[index + 1:min(index+step, len(bhb_fits))]:
			q_str = q_str + ' OR (lines.bestObjID = ' + `bhb['spec.bestObjID']` + ' AND lines.specObjID = ' + `bhb['spec.specObjID']` + ')'
...
def createFits(bhb_fits, results, fields, formats, filename):
	...
	width = len(results[0][2:]) # how many individual fields were queried
	...
	for i in range(len(bhb_fits)):
		...
		for res in results:
			if bhb_fits[i]['spec.bestObjID'] == res[0]:
				indices.append(i)
...
def main():
	...
	bhb_fits = fits.open('combined_ha24.fits')[1].data

	start = 0
	end = len(bhb_fits)

	fields,formats = readFieldsFile('balmerFields.txt')
	...
	createFits(bhb_fits, results, fields[2:], formats[2:], 'combined_022317.fits')
